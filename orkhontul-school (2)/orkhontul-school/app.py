...
def row_to_dict(row):
    d = dict(row)
    if d.get("option_a"):
        d["options"] = [f"А. {d['option_a']}", f"Б. {d['option_b']}", f"В. {d['option_c']}", f"Г. {d['option_d']}"]
    else:
        d["options"] = None
    return d
...
def generate_exam():
    ...
    if blueprint:
        ...
        limit = 40 if exam_type == "Улсын шалгалт" else 25
        rows  = conn.execute(
    ...
    exam = {
        ...
        "grade":           grade, "subject": subject, "exam_type": exam_type,
...
def stats():
    ...
    conn.close()
    return jsonify({"total_questions": total or 0, "grades": grades or 12, "subjects": subjects or 10, "blueprints": 36})
